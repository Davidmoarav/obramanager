...
export default function SelectorCatalogo({ proveedorId, onPick }: { proveedorId: string; onPick: (p: any) => void }) {
  const [q, setQ]       = useState('')
  const [open, setOpen] = useState(false)
  const { data: productos = [] } = useSWR<any[]>(
    proveedorId && open
      ? `/api/proveedor-productos?proveedor_id=${proveedorId}${q.trim() ? `&buscar=${encodeURIComponent(q.trim())}` : ''}`
      : null,
    fetcher,
  )
  ...
  return (
    <div className="relative mb-3">
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        + Agregar del catálogo del proveedor
      </button>
      {open && (
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-line rounded-lg shadow-lg max-h-[280px] overflow-y-auto">
          ...
          {productos.length === 0
            ? <div className="px-3 py-4 text-[11px] text-muted text-center">{q.trim() ? 'Sin resultados' : 'Este proveedor no tiene catálogo cargado'}</div>
            : productos.map(p => (
